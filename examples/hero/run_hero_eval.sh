#!/usr/bin/env bash
# HERO evaluation script — reproduces arXiv:2510.07242v1 evaluation protocol.
#
...
#
# Usage:
#   # Evaluate a trained HERO checkpoint
...
output_dir=${HERO_OUTPUT_DIR:-$HOME/data/hero_eval_results/$(basename "$model_path")_$(date +%Y%m%d_%H%M%S)}

# ─── Benchmarks to evaluate ──────────────────────────────────────────
all_benchmarks=${HERO_EVAL_BENCHMARKS:-"math500 amc minerva olympiad hardverify_math textbook_reasoning"}

# ─── Generation settings (Section 4.1) ───────────────────────────────
n_samples=${HERO_EVAL_N_SAMPLES:-8}
temperature=${HERO_EVAL_TEMPERATURE:-0.6}
top_p=${HERO_EVAL_TOP_P:-0.95}
max_prompt_length=${HERO_EVAL_MAX_PROMPT_LENGTH:-1024}
max_response_length=${HERO_EVAL_MAX_RESPONSE_LENGTH:-8192}
tp_size=${HERO_EVAL_TP_SIZE:-2}
gpu_memory_utilization=${HERO_EVAL_GPU_MEMORY_UTILIZATION:-0.85}
gpus_per_node=${HERO_EVAL_GPUS_PER_NODE:-8}
nnodes=${HERO_EVAL_NNODES:-1}
primary_response_index=${HERO_EVAL_PRIMARY_RESPONSE_INDEX:-0}

# ─── LLM-as-judge settings ───────────────────────────────────────────
judge_model=${HERO_JUDGE_MODEL:-gpt-oss}
judge_base_url=${HERO_JUDGE_BASE_URL:-https://ellm.nrp-nautilus.io/v1}
judge_concurrency=${HERO_JUDGE_CONCURRENCY:-32}
...
# ═══════════════════════════════════════════════════════════════════════
# Step 3: Score hard-to-verify benchmarks with GPT-4o judge
# ═══════════════════════════════════════════════════════════════════════
if [[ -n "$hard_benchmarks" ]]; then
    ...
    for bench in $hard_benchmarks; do
        ...
        python3 examples/hero/eval_hero_llm_judge.py "${judge_args[@]}" \
