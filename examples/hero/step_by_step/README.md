...
- You can still override any underlying env var, such as `HERO_MAX_RESPONSE_LENGTH`, `HERO_ROLLOUT_N`, `HERO_RM_TP_SIZE`, or `HERO_DEBUG`.
- Set `HERO_DEBUG=1` to enable debug mode, which uses only the `amc` benchmark for validation during RL training to speed up iteration.
