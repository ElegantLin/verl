#!/usr/bin/env bash
# HERO cold-start SFT wrapper.
#
# This reproduces the paper's cold-start stage by:
...
input_path=${HERO_SFT_INPUT_PATH:-$HOME/data/hero_source/source_generated.parquet}
sft_data_dir=${HERO_SFT_DATA_DIR:-$HOME/data/openmathreasoning_hero_sft}
output_dir=${HERO_SFT_OUTPUT_DIR:-checkpoints/hero_paper_reproduction/hero_cold_start_sft}
...
prompt_col=${HERO_PROMPT_COL:-prompt}
reward_model_col=${HERO_REWARD_MODEL_COL:-reward_model}
response_col=${HERO_RESPONSE_COL:-responses}
primary_response_index=${HERO_PRIMARY_RESPONSE_INDEX:-0}
num_samples=${HERO_SFT_NUM_SAMPLES:-2000}
val_size=${HERO_SFT_VAL_SIZE:-0}
seed=${HERO_SEED:-42}
max_response_chars=${HERO_SFT_MAX_RESPONSE_CHARS:-12000}
force_preprocess=${HERO_FORCE_SFT_PREPROCESS:-0}

nnodes=${HERO_SFT_NNODES:-1}
nproc_per_node=${HERO_SFT_GPUS_PER_NODE:-8}
train_batch_size=${HERO_SFT_TRAIN_BATCH_SIZE:-64}
micro_batch_size=${HERO_SFT_MICRO_BATCH_SIZE_PER_GPU:-1}
max_length=${HERO_SFT_MAX_LENGTH:-9216}
...
loggers=${HERO_SFT_LOGGERS:-'["console"]'}
experiment_name=${HERO_SFT_EXPERIMENT_NAME:-hero_cold_start_sft_$(date +%Y%m%d_%H%M%S)}
...
if [[ "$nnodes" == "1" ]]; then
    ...
    torchrun_cmd+=(--node_rank="${HERO_SFT_NODE_RANK:-0}")
    torchrun_cmd+=(--master_addr="${HERO_SFT_MASTER_ADDR:?Set HERO_SFT_MASTER_ADDR for multi-node cold-start SFT}")
    torchrun_cmd+=(--master_port="${HERO_SFT_MASTER_PORT:-29500}")
fi
